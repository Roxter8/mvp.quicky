...
export function MarketCard({ market, onClick }: MarketCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 rounded-2xl p-4 cursor-pointer hover:border-zinc-700 transition-all active:scale-[0.98]"
    >
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-zinc-300">{formatVolume(market.volume)}</span>
      </div>
    </div>
  );
}
